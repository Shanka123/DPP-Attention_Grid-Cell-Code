...
def train(args, model,sigma, device, train_loader,analogy_levels_loc,optimizer, epoch):
	...
	for batch_idx, (ABC, D_choices, D_correct) in enumerate(train_loader):
		...
		D_scores= model(torch.flatten(ABC_gaussian,start_dim=2,end_dim=3), torch.flatten(D_choices_gaussian,start_dim=2,end_dim=3), device)
...
def val(args, model,sigma, device, val_loader,analogy_levels_loc):
	# Set to training mode
...
def test(args,t,  model,sigma, device, loader,analogy_levels_loc):
...
def main():
	...
	parser.add_argument('--model_name', type=str, default='analogy_smoothed_onehots_lstm_module' )
	...
	for epoch in range(1, args.epochs + 1):
		# Training loop
		loss, acc = train(args, model,args.sigma, device, train_loader, analogy_levels_loc,optimizer, epoch)
		# all_loss.append(loss)
		# all_acc.append(acc)
		valacc = val(args, model,args.sigma, device, val_loader,analogy_levels_loc)
		if valacc > max_val_acc:
			print("Average validation accuracy increased from %s to %s"%(max_val_acc,valacc))
			max_val_acc = valacc
			# print("Saving model$$$$$")
			# save_nw(model,optimizer,epoch,'{}_{}_sigma_{}_run_{}_best.pth.tar'.format(args.model_name, args.extrap_regime,args.sigma,args.run))
			temp_test_acc=[]
			for t in range(len(test_loaders)):
				...
				acc = test(args,t, model,args.sigma, device, test_loaders[t],analogy_levels_loc)
