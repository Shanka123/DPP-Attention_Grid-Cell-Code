...
def train(args, model,enc_model, device, train_loader,analogy_levels_loc,optimizer, epoch):
	...
	for batch_idx, (ABC, D_choices, D_correct) in enumerate(train_loader):
		...
		A_latent = enc_model(ABC_onehots[:,0])
		B_latent = enc_model(ABC_onehots[:,1])
		C_latent = enc_model(ABC_onehots[:,2])
		total_latent = torch.cat((total_latent,A_latent.unsqueeze(1),B_latent.unsqueeze(1),C_latent.unsqueeze(1)),dim=1)
		# all_choice_latent= torch.Tensor().to(device).float()
		for d in range(D_choices.shape[1]):
			total_latent = torch.cat([total_latent,enc_model(D_choices_onehots[:,d,:]).unsqueeze(1)],dim=1)
		...
		D_scores= model(torch.cat((A_latent.unsqueeze(1),B_latent.unsqueeze(1),C_latent.unsqueeze(1)),dim=1), D_choices_onehots,enc_model, device)
...
def val(args, model,enc_model, device, val_loader,analogy_levels_loc):
	# Set to training mode
...
def test(args,t,  model,enc_model, device, loader,analogy_levels_loc):
...
def main():
	...
	parser.add_argument('--model_name', type=str, default='analogy_dppa_onehots_encoder_lstm_module' )
	...
	# devs_cellwise = torch.Tensor([0.        , 0.        , 0.        , 0.        , 0.08333333,
	...
	for epoch in range(1, args.epochs + 1):
		# Training loop
		loss, acc = train(args, model,enc_model, device, train_loader, analogy_levels_loc,optimizer, epoch)
		# all_loss.append(loss)
		# all_acc.append(acc)
		valacc = val(args, model, enc_model,device, val_loader,analogy_levels_loc)
		if valacc > max_val_acc:
			print("Average validation accuracy increased from %s to %s"%(max_val_acc,valacc))
			max_val_acc = valacc
			# print("Saving model$$$$$")
			# save_nw(enc_model,model,optimizer,epoch,'{}_{}_run_{}_best.pth.tar'.format(args.model_name, args.extrap_regime,args.run))
			temp_test_acc=[]
			for t in range(len(test_loaders)):
				...
				acc = test(args,t, model,enc_model, device, test_loaders[t],analogy_levels_loc)
