...
class LSTM(nn.Module):
	...
	def forward(self, x_seq, device):
		...
		# x_seq_new = torch.cat((x_seq[:,:,:x_seq.shape[2]-2] *  torch.unsqueeze(torch.unsqueeze(weights,0),0) ,x_seq[:,:,x_seq.shape[2]-2:]),dim=2)
...
class Lstm_scoring_model(nn.Module):
	def __init__(self,in_dim, mlp_encoder=False, multiLSTM=False):
		super(Lstm_scoring_model, self).__init__()
		...
		if self.mlp_encoder:
			...
			self.lstm = LSTM(in_dim +2, num_layers=num_layers_LSTM)
...
def train(args,weights_model, lstm_scoring_model, device, train_loader, grid_codes,L_kernel,within_freq_dets_tensor,N_freqs,addition_levels_loc,optimizer, epoch):
	...
	for batch_idx, (AB, C_choices, C_correct,one_hots) in enumerate(train_loader):
		...
		# D_choices = grid_codes[analogy_levels_loc[D_choices],:]
			...
			C_scores= lstm_scoring_model(AB, C_choices, one_hots,device)
...
def val(args,lstm_scoring_model, device, val_loader, grid_codes,within_freq_dets_tensor, addition_levels_loc,epoch):
...
def test(args,t,  lstm_scoring_model, device, loader, grid_codes,within_freq_dets_tensor, addition_levels_loc,epoch):
	# Set to evaluation mode
	lstm_scoring_model.eval()
	# Iterate over batches
	all_acc = []
...
def save_nw(weights_model,lstm_scoring_model,optimizer,epoch, name):
	...
	torch.save({
		...
		'epoch': epoch,
	}, '/scratch/gpfs/smondal/dpp_extrapolation/weights/'+name)
	

def load_checkpoint(weights_model,lstm_scoring_model, checkpoint_path):
	"""
	...
	return weights_model,lstm_scoring_model	
...
def main():
	...
	parser.add_argument('--model_name', type=str, default='arithmetic_dppa_gridcellcode_lstm_module' )
	...
	# original_grid_codes = mat73.loadmat('/scratch/gpfs/smondal/dpp_extrapolation/12freqs_grid_codes_1000x1000.mat')['GC_FRmaps']
	...
	print("Number of frequencies>>>", N_freqs)
 ...
 #        1.9779e+10, 1.2334e+10, 1.5305e+10]).to(device)
	...
	lstm_scoring_model = Lstm_scoring_model(N_grid_cells//N_freqs, mlp_encoder=False, multiLSTM=False).to(device)
	...
	optimizer = optim.Adam(list(weights_model.parameters())+list(lstm_scoring_model.parameters()), lr=args.lr)
	...
	for epoch in range(1, args.epochs + 1):
		...
		if epoch <= args.epochs_maximizedet:
			...
			loss, train_withinfreq_dets = train(args, weights_model,lstm_scoring_model, device, train_loader, grid_codes,L_kernel,train_withinfreq_dets,N_freqs, addition_levels_loc,optimizer, epoch)
		

			print("Within frequency log determinant returned after training block for training region >>>", train_withinfreq_dets)
		...
		# if epoch > args.epochs_maximizedet:
			...
			loss = train(args, weights_model,lstm_scoring_model, device, train_loader, grid_codes,L_kernel,train_withinfreq_dets,N_freqs, addition_levels_loc,optimizer, epoch)
		

			valacc,valadd_acc,valmul_acc = val(args, lstm_scoring_model, device, val_loader, grid_codes,train_withinfreq_dets,addition_levels_loc,epoch)
			if valacc > max_val_acc:
				...
				# print("Saving model$$$$$")
				# save_nw(weights_model,lstm_scoring_model,optimizer,epoch,'{}_run_{}_best.pth.tar'.format(args.model_name,args.run))
				...
				for t in range(len(test_loaders)):
					...
					acc, add_acc, mul_acc= test(args,t, lstm_scoring_model, device, test_loaders[t], grid_codes,train_withinfreq_dets,addition_levels_loc,epoch)
